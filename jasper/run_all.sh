...
for face1 in {0,2}; do
    for face2 in {0,2}; do
        ...
        for fid in {0..4}; do
            qsub -N fiducial_"$fid"_"$face1"_"$face2" -l nodes=$NODE:ppn=$PROCS,pmem=$PMEM,walltime=$HOURS:00:00 -v SCRIPT_PATH=$SCRIPT_PATH,FIDUCIAL="fid_comp",FACE1=$face1,FACE2=$face2,DATA_DIR=$DATA_DIR,ADD_NOISE=$ADD_NOISE,RESULTS_DIR=$RESULTS_DIR $SCRIPT_PATH/jasper/fiducial_submit.pbs
        done
    done
done
...
for face1 in {0,2}; do
    for face2 in {0,2}; do
        ...
        for fid in {0..4}; do
            qsub -N fiducial_noise_"$fid"_"$face1"_"$face2" -l nodes=$NODE:ppn=$PROCS,pmem=$PMEM,walltime=$HOURS:00:00 -v SCRIPT_PATH=$SCRIPT_PATH,FIDUCIAL="fid_comp",FACE1=$face1,FACE2=$face2,DATA_DIR=$DATA_DIR,ADD_NOISE=$ADD_NOISE,RESULTS_DIR=$RESULTS_DIR $SCRIPT_PATH/jasper/fiducial_submit.pbs
        done
    done
done
